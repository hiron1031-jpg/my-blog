...
function getMessage(correct: number, total: number): string {
  ...
  if (pct >= 0.7) return "合格ラインに到達！この調子でほかの年度もチャレンジしよう！";
...
export default function QuizClient({
  ...
  function retryQuiz() {
    launchYearQuiz(selectedExam, selectedYear, selectedSection, "all", totalQuestions);
  }
  ...
  if (phase === "select-section") {
    const allExamConfigs = CATEGORY_CONFIGS.flatMap((c) => c.exams);
    const examConfig = allExamConfigs.find((e) => e.examKey === selectedExam);
                        ...
                        {years.map((year) => {
                          ...
                          return (
                            <button
                              ...
                              className={`flex flex-col items-start px-4 py-3.5 rounded-2xl bg-gradient-to-br ${examConfig.gradient} ${examConfig.hover} transition-all duration-200 shadow-md hover:shadow-lg hover:-translate-y-0.5 text-white text-left`}
                            >
                              <p className="font-bold text-base">{year}年度</p>
                              ...
                              {bestScore ? (
                                ...
                                <p className="text-white/40 text-xs mt-1.5">未挑戦</p>
                              )}
                            </button>
                          );
                        })}
            ...
            <div className="bg-surface border border-border rounded-xl p-4 mb-6 text-left">
              ...
              <div className="flex flex-col gap-1.5 text-sm">
                ...
                {PACK_LINKS[selectedExam] ? (
                  ...
                  >
                    二次対策の教材をまとめた「合格パック」（
                    {PACK_LINKS[selectedExam].price}・単品より
                    {PACK_LINKS[selectedExam].save}お得）→
                  </a>
                ) : (
                  <a
                    href={NOTE_KAISETSU[selectedExam].url}
